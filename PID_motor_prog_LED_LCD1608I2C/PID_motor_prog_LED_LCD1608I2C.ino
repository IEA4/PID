...
void loop(){
    while(true){
        while(digitalReadFast(REGIM_PIN) == 1){           // РЕЖИМ ПРОГРАММНОЙ РЕГУЛИРОВКИ. Откачка согласно программе, по ступенькам по заданным интервалам времени
            ...
            lcd.setCursor(0,0);    lcd.print(F("P_"));      // Программный
...
void auto_regim(){
    ...
    for (i=0; i<12; i++){                                // 12 ступеней откачки
        lcd.setCursor(4,0);     lcd.print(F("12|"));
        lcd.setCursor(7,0);     lcd.print(i+1);                 // выводим на первой строчке номер ступени, начиная от 1
        ...
        if (i == 3 || i == 8){                      // на этих ступенях откачка останавливается для возможности перевернуть рамки
            ...
            uint16_t tmr_delta, tmr_deltaLast;
            uint32_t tmrbO = millis();

            lcd.setCursor(13,0);    lcd.print((unsigned int)(0.001 * (pgm_read_word(&time_stup_arr[j][i]))));   //обратный отсчт на дисплее
            if  (pgm_read_word(&time_stup_arr[j][i]) < 10000){
                lcd.setCursor(11,0);    lcd.print((unsigned int)(0.001 * (pgm_read_word(&time_stup_arr[j][i]))));
            }
            else {
                lcd.setCursor(10,0);    lcd.print((unsigned int)(0.001 * (pgm_read_word(&time_stup_arr[j][i]))));
            }
            lcd.setCursor(15,0);    lcd.print(F(" "));   //костыль чтоб не выводились левые символы

            while(millis() - tmrbO < pgm_read_word(&time_stup_arr[j][i])){
                ...
                if(f_AbO == 0){         //обратный отсчёт оставшегося времени откачки
                    tmr_delta = (unsigned int)(0.001 * ((pgm_read_word(&time_stup_arr[j][i])) - (millis() - tmrbO)));
...
void pid_in_auto(){
    if (f_AuL == 1){
        lcd.setCursor(4,1);    lcd.print(pgm_read_word(&stup_arr[j][i]));
